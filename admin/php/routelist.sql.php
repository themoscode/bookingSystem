...
<h3>LINIEN & FAHRTEN</h3>
<div id="newRouteMain">
	
	<fieldset class="fieldOn">
		<legend>Neue Linien</legend>
		<input type="button" value="Neue Linie eintragen" id="butNewRoute" class="butNewRoute">	
	</fieldset>
		
	<fieldset class="fieldOff" id="newRoute">
		<legend>Haltestellen:</legend>
		';
	...
	'</select>
		...
		<div id="tdSelectStations" class="fieldOff">';
		...
		<input type="button" value="Linie speichern" id="butSaveNewRoute" disabled >
	
	</fieldset>
</div>';

	
	$ergebnis["result"] = $ergebnis["result"] .'<div id="ridesMainTableWrap">
	<legend>Fahrten-Menü</legend>';
	foreach ($routes as $index=>$route): 
						...
						<tr>
							...
							<td>
								...
								if ($numberOfUniqueRoutes != 0) {
									...
									$ergebnis["result"] = $ergebnis["result"] . 'Fahrten von: <input type="text" maxlength="10" class="dateField_search" id="searchDateFrom_'.$route['routeID'].'" value="'.$searchDateFrom.'">
									bis: <input type="text" maxlength="10" class="dateField_search" id="searchDateTo_'.$route['routeID'].'" value="'.$searchDateTo.'"> 
									<input type="button" value="Datenbankabfrage" id="uniqueRouteSearch_'.$route['routeID'].'" onclick=myRoute.getRouteList("'.$route['routeID'].'");>
									';
								}
				$ergebnis["result"] = $ergebnis["result"] .
								'<input type="button" value="Neue Fahrt" class="newRideButton" id="newUniqueRoute_'.$route['routeID'].'" onclick="myRoute.newUniqueRoute('.$route['routeID'].');">
							</td>
						</tr>';
				...
				$sql2='SELECT
					...
					unique_route.departure AS rideDeparture,
			...
			if ($countRouteStationDetails > 0) {		
					...
					//$stationOrderStr=substr()
					
					$stationOrderStr = substr($stationOrderStr,0,strlen($stationOrderStr)-3);
						...
						'<tr>
							<td colspan="3">
								<fieldset class="'.$fieldClass.'" onOff ="on" id="routeStationDetails_'.$route['routeID'].'">
									<h2 class="routeListHeader">Fahrten der Linie: '.$stationOrderStr.' </h2>
									<table>
										<tr>
											<th>VON</th>
											<th>NACH</th>
								...
								foreach ($routeStationDetails as $index2=>$routeStationDetail): 
										...
										$rideDeparture = strtotime($routeStationDetail['rideDeparture']);
										$curtime = time();
										...
										$saveButtons = '
										<input type="button" '.$disabled_buttons.' value="Fahrt speichern" data-routeid = "'.$route['routeID'].'"  data-routeStationDetailsIds="'.$routeStationDetailsIds.'" data-uniquerouteid = "'.$uniqueRouteID.'"  class="editRouteStationDetailBut"  id="editRouteStationDetailBut_'.$uniqueRouteID.'">
										<input type="button" '.$disabled_buttons.' value="Fahrt online stellen" data-routeid = "'.$route['routeID'].'"  data-routeStationDetailsIds="'.$routeStationDetailsIds.'" data-uniquerouteid = "'.$uniqueRouteID.'"  class="editRouteStationDetailBut_online"  id="editRouteStationDetailBut_online_'.$uniqueRouteID.'">';
										...
										if ($routeStationDetail['active'] == 1) {
											$disabled = ' disabled ';
											$saveButtons = '<p class="onlineinfo'.$pastRideClass.'">ONLINE</p>
											<input type="button"  '.$disabled_buttons.' value="Fahrt speichern" data-routeid = "'.$route['routeID'].'"  data-routeStationDetailsIds="'.$routeStationDetailsIds.'" data-uniquerouteid = "'.$uniqueRouteID.'"  class="editRouteStationDetailBut"  id="editRouteStationDetailBut_'.$uniqueRouteID.'">
											<input type="button"  '.$disabled_buttons.' value="Fahrt stornieren" data-routeid = "'.$route['routeID'].'"  data-routeStationDetailsIds="'.$routeStationDetailsIds.'" data-uniquerouteid = "'.$uniqueRouteID.'"  class="editRouteStationDetailBut_cancel"  id="editRouteStationDetailBut_cancel_'.$uniqueRouteID.'" data-bookings="'.getBookingsNum($routeStationDetailsIds,$myPDO).'">';
										}
										elseif ($routeStationDetail['active'] == -1) {
											$disabled = ' disabled ';
											$disabled_price=' disabled ';
											$saveButtons = '<p class="cancelinfo'.$pastRideClass.'">STORNIERT</p>';
									...
									$ergebnis["result"] = $ergebnis["result"] .		
										...
										if ($routeStationDetailCounter >= $numSingleRides) {
											...
											$ergebnis["result"] = $ergebnis["result"] .		
												'<tr class="'.$pastRideClass.'">
													<td colspan=7 class="carinfo">
														Fahrzeug:
												...
												'<tr class="'.$pastRideClass.'"><td colspan=7 align="center">'.$saveButtons.'</td></tr>';
	...
	$ergebnis["result"] = $ergebnis["result"] .	'</table>';
	$ergebnis["result"] = $ergebnis["result"] . '</div>';
